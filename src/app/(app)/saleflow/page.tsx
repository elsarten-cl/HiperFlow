...
export default function SaleFlowPage() {
  ...
  const [editingDeal, setEditingDeal] = useState<WithId<Deal> | null>(null);
  ...
  const handleOpenEditDeal = (deal: WithId<Deal>) => {
    setEditingDeal(deal);
    setIsSheetOpen(true);
  }
  ...
  const handleSaveDeal = async (formData: Partial<Deal>) => {
    ...
    if (editingDeal) {
      ...
      const dealRef = doc(firestore, 'deals', editingDeal.id);
      const updatedData = { ...formData, updatedAt: serverTimestamp() };
      updateDocumentNonBlocking(dealRef, updatedData);
      toast({ title: "Flow Actualizado", description: "La oportunidad ha sido actualizada." });
  ...
  return (
    <>
      ...
      <KanbanBoard onEditDeal={handleOpenEditDeal} />

      <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
        <SheetContent className="sm:max-w-lg">
          <SheetHeader>
            <SheetTitle>{editingDeal ? 'Editar Flow' : 'Crear Nuevo Flow'}</SheetTitle>
            <SheetDescription>{editingDeal ? 'Actualiza los detalles de esta oportunidad.' : 'Inicia un nuevo negocio para añadir a tu pipeline.'}</SheetDescription>
          </SheetHeader>
          <div className="py-4">
            <DealForm
